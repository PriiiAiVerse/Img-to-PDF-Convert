...
def convert():
    ...
    root.location = tkinter.filedialog.asksaveasfilename(initialdir="/", title="Select file",
...
def get_Split_file_func():
    root.get_split_filename = tkinter.filedialog.askopenfilename(initialdir="/", title="Select file",
...
def get_first_pdf_fn():
    root.first_pdf_file = tkinter.filedialog.askopenfilename(initialdir="/", title="Select file",
                                                             filetypes=(("PDF files", "*.PDF"), ("all files", "*.*")))
...
def get_second_pdf_fn():
    root.second_pdf_file = tkinter.filedialog.askopenfilename(initialdir="/", title="Select file",
                                                              filetypes=(("PDF files", "*.PDF"), ("all files", "*.*")))
...
def merge():
    ...
    root.filename = tkinter.filedialog.asksaveasfilename(initialdir="/", title="Select file",
                                                         filetypes=(("PDF files", "*.PDF"), ("all files", "*.*")))
    print((root.filename))
...
x = Radiobutton(root, bg='#ffffff', text="Convert", variable=option, value=1, command=select1, padx=30, pady=50).grid(
    column=0, row=0)
y = Radiobutton(root, bg='#ffffff', text="Split", variable=option, value=2, command=select2, padx=30, pady=50).grid(
    column=1, row=0)
z = Radiobutton(root, bg='#ffffff', text="Join", variable=option, value=3, command=select3, padx=30, pady=50).grid(
    column=2, row=0)
